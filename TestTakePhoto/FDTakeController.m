...
//      self.imagePicker.videoQuality = UIImagePickerControllerQualityTypeMedium; // default medium
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    // Handle a movie capture
      ...
      // todo
        ...
        if (self.autoConvertVideo) {
          UIAlertView * loadAlert = [[UIAlertView alloc] init];
          [loadAlert setTitle:@"转码中..."];
          UIActivityIndicatorView* activity = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge];
          activity.frame = CGRectMake(140,80,CGRectGetWidth(loadAlert.frame),CGRectGetHeight(loadAlert.frame));
          [loadAlert addSubview:activity];
          [activity startAnimating];
          [loadAlert show];
          
//          NSLog(@"--------- mov %@ ----------",[self getFileSizeAndDurationString:info[UIImagePickerControllerMediaURL]]);
          [self transToMP4WithUrl:info[UIImagePickerControllerMediaURL] complete:^(NSURL *fileUrl) {
            //            mp4Url = [NSURL URLWithString:[NSString stringWithFormat:@"file://localhost/private%@", path]];
            [loadAlert dismissWithClickedButtonIndex:0 animated:YES];
//            NSLog(@"--------- mp4 %@ ----------",[self getFileSizeAndDurationString:fileUrl]);
            [self.delegate takeController:self gotVideo:fileUrl withInfo:info];
            //          [self removeFileWithUrl:fileUrl];
          }];
        }else{
...
- (UIViewController*)presentingViewController
{
    ...
    if (self.viewControllerForPresentingImagePickerController!=nil) {
      ...
      self.viewControllerForPresentingImagePickerController = presentingViewController;
    }
...
-(void)transToMP4WithUrl:(NSURL*)videoURL complete:(void(^)(NSURL *fileUrl))theBlock
{
...
//    NSString *mp4Path = [NSHomeDirectory() stringByAppendingFormat:@"/Documents/output-%@.mp4", [formater stringFromDate:[NSDate date]]];
    NSURL *mp4Url = [[NSURL fileURLWithPath:NSHomeDirectory() isDirectory:YES] URLByAppendingPathComponent:[NSString stringWithFormat:@"/Documents/output-%@.MP4",[formater stringFromDate:[NSDate date]]]];
...
-(BOOL)removeFileWithUrl:(NSURL *)theUrl
{
  NSFileManager * filemanager = [NSFileManager defaultManager];
  NSError *error;
  BOOL result = [filemanager removeItemAtURL:theUrl error:&error];
  if (error) {
    result = NO;
  }
  return result;
...
-(NSString *)getFileSizeAndDurationString:(NSURL*) URL
{
  return [NSString stringWithFormat:@"size:%.2f kb,duration:%.1f s",[self getFileSizeWithUrl:URL],[self getMovieDuration:URL]];
}

-(float)getFileSizeWithUrl:(NSURL*) URL
{
  // size
...
-(float)getMovieDuration:(NSURL*) URL
{
  // duration
